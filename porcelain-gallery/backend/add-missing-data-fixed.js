...
async function addMissingData() {
  ...
  try {
    ...
    // 2. 更新现有朝代的sort_order，为Yuan Dynasty腾出位置
    console.log('更新朝代排序...');
    ...
    // 6. 更新产品的primary_image_id
    const [yuanImageResult] = await connection.execute('SELECT LAST_INSERT_ID() as id');
    const yuanImageId = yuanImageResult[0].id - 1;
    const mingImageId = yuanImageResult[0].id;
